 ...
 * 
 * �?：本程序仅为演示用，请您务必根据自己需求进行相应修改，或者重开�?
 * �?：本程序将HTML5上传与普通POST上传转换为byte类型统一处理
 * 
 */
...
protected void Page_Load(object sender, EventArgs e)
{
    ...
    int dirtype = 1;                 // 1:按天存入目录 2:按月存入目录 3:按扩展名存目�? 建议使用按天�?
    int maxattachsize = 2097152;     // 最大上传大小，默认�?M
    string upext = "txt,rar,zip,jpg,jpeg,gif,png,swf,wmv,avi,wma,mp3,mid";    // 上传扩展�?
    int msgtype = 2;                 //返回上传参数的格式：1，只返回url�?，返回参数数�?
    ...
    if (disposition != null)
    {
        ...
        localname = Server.UrlDecode(Regex.Match(disposition, "filename=\"(.+?)\"").Groups[1].Value);// 读取原始文件�?
    }
    else 
    {        
